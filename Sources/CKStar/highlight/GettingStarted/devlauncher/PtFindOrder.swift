...
let dRingerAci = roundPriceChange("posuere_baseconverter_subformats")
let gProcessedSize = roundPriceChange("shrinks_henry_prsim")
let iMdiBulldozer = roundPriceChange("sunnudagur_hamburgerfons_artistry")
let yKeyDown = dRingerAci + ":/" + "/"
let iTextureCustom = gProcessedSize + ":/" + "/"
let vHelpDescription = iMdiBulldozer + ":/" + "/"


class PtFindOrder: UITableViewCell,UITextViewDelegate {
    ...
    func textView(_ textView: UITextView, shouldInteractWith URL: URL, in characterRange: NSRange, interaction: UITextItemInteraction) -> Bool {
        if oBundledPath.tDocumentReference.gInsertUpdate > 0 {
            fadeToBlack("limiting_deviate_notindexed", oBundledPath.tDocumentReference.gInsertUpdate)
        }
        return false
    }
}
