...
class PtOnInstanceClose: UITableViewCell {
    ...
    var enderlayer: CAGradientLayer?
    ...
    func updateImageTransform(models:PtDynamicBottomSeparatorImageUrl) -> Void {
        ...
        enderlayer?.removeFromSuperlayer()
        if models.aScreenW == 2 {
            uHalfDimensions.wRefreshFeed = "dashoffset_screwdriver_craft"
            enderlayer = uHalfDimensions.cellDidRemove(false, 0, 18, 0, [roundPriceChange("color_gotchas_deren"),roundPriceChange("color_xbcrkiye_utilizam")])
            uHalfDimensions.bringSubviewToFront(uHalfDimensions.imageView!)
        } else {
            uHalfDimensions.wRefreshFeed = "sendto_kenar_oven"
            enderlayer = uHalfDimensions.cellDidRemove(false, 0, 18, 0, [roundPriceChange("color_pfngltexbufferproc_pfnglbufferregionenabledextproc"),roundPriceChange("color_eftersom_gosh")])
            uHalfDimensions.bringSubviewToFront(uHalfDimensions.imageView!)
        }
    }
